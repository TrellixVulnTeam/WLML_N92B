...
if __name__ == '__main__':
    ...
    def network(sample):
        ...
        video_score = sample['batch_scores'].cuda()
        video_label = sample['batch_gt_pairs'].cuda()

        logits_text, logits_iou, iou_mask_map, gt_times, is_single = model(textual_input, textual_mask, word_mask, visual_input, video_score, video_label)
        loss_value, iou_scores, regress = getattr(loss, config.LOSS.NAME)(config.LOSS.PARAMS, model.training, logits_text, logits_iou, iou_mask_map, word_label, word_mask, gt_times, is_single, video_label)
    ...
    def get_proposal_results(iou_scores, regress, durations):
        out_sorted_times = []
        T = iou_scores.shape[-1] - 1
        N = iou_scores.shape[0]

        scores = iou_scores
        regress = regress.cpu().detach().numpy()

        for score, reg, duration in zip(scores, regress, durations):
            sorted_indexs = np.dstack(np.unravel_index(np.argsort(score.cpu().detach().numpy().ravel())[::-1], (T, T+1))).tolist()
    ...
    def on_start(state):
        ...
        model.train()
        if config.VERBOSE:
            state['progress_bar'] = tqdm(total=state['test_interval'], ncols=80)
    ...
    def on_test_start(state):
        ...
        if config.VERBOSE:
            if state['split'] == 'train':
                state['progress_bar'] = tqdm(total=math.ceil(len(train_dataset)/config.TEST.BATCH_SIZE), ncols=80)
            elif state['split'] == 'val':
                state['progress_bar'] = tqdm(total=math.ceil(len(val_dataset)/config.TEST.BATCH_SIZE), ncols=80)
            elif state['split'] == 'test':
                state['progress_bar'] = tqdm(total=math.ceil(len(test_dataset)/config.TEST.BATCH_SIZE), ncols=80)
            else:
                raise NotImplementedError
