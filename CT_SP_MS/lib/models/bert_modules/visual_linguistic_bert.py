...
class PositionalEncoding(nn.Module):
    ...
    def __init__(self, d_hid, n_position=32):
...
class VisualLinguisticBert(BaseModel):
    def __init__(self, config, language_pretrained_model_path=None):
        ...
        self.cls_sep_end_embedding = nn.Embedding(3, config.hidden_size)
        self.token_type_embeddings = nn.Embedding(config.type_vocab_size, config.hidden_size)
        self.embedding_LayerNorm = BertLayerNorm(config.hidden_size, eps=1e-12)
        self.embedding_dropout = nn.Dropout(config.hidden_dropout_prob)

        self.visual_word_mapping = nn.Linear(300, config.hidden_size)

        self.postion_encoding = PositionalEncoding(config.hidden_size, config.n_position)
    ...
    def forward(self,
        ...
        embedding_output, attention_mask, text_mask_new, object_mask_new = self.embedding(text_input_feats,
                                                                                          text_visual_embeddings,
                                                                                          text_mask,
                                                                                          word_mask,
                                                                                          object_visual_embeddings,
        ...
        if output_text_and_object_separately:
            ...
            for encoded_layer in encoded_layers:
                max_text_len = text_input_feats.shape[1]
                max_object_len = object_visual_embeddings.shape[1]-2
                encoded_layer_object = encoded_layer[:, 1:max_object_len+1]
                encoded_layer_text = encoded_layer.new_zeros(
                    (encoded_layer.shape[0], max_text_len, encoded_layer.shape[2]))
                encoded_layer_text[text_mask>0] = encoded_layer[text_mask_new]
    ...
    def embedding(self,
        ...
        object_linguistic_embeddings = self.visual_word_mapping(text_input_feats_temp.sum(1, keepdim=True)/(mask_word_mean.sum(1)[:,None,None]))
        ...
        text_vl_embeddings = text_linguistic_embedding + text_visual_embeddings[:,:-1,:]
        ...
        object_vl_embeddings = object_linguistic_embeddings + object_visual_embeddings[:,1:-1,:]

        bs = text_vl_embeddings.size(0)
        vl_embed_size = text_vl_embeddings.size(-1)
        max_length = int((text_mask.sum(1) + object_vl_embeddings.shape[1]).max().item()) + 3
        ...
        object_end = object_vl_embeddings.shape[1] + 1
        text_end = object_end + 1 + text_mask.sum(1, keepdim=True).long()
        ...
        _one_id = torch.zeros((bs, ), dtype=torch.long, device=text_vl_embeddings.device)
        vl_embeddings = text_vl_embeddings.new_zeros((bs, max_length, vl_embed_size))
        vl_embeddings[:,0,:] = self.cls_sep_end_embedding(_zero_id) + object_visual_embeddings[:,0,:]
        vl_embeddings[(grid_pos > 0) & (grid_pos < object_end)]  = object_vl_embeddings.view(-1,object_vl_embeddings.size(-1))
        vl_embeddings[:,object_end,:]  = self.cls_sep_end_embedding(_one_id) + object_visual_embeddings[:,-1,:]
        vl_embeddings[(grid_pos > object_end) & (grid_pos < text_end)] = text_vl_embeddings[text_mask>0]
        vl_embeddings[grid_pos == text_end] = self.cls_sep_end_embedding(_one_id + _one_id) + text_visual_embeddings[:,-1,:]

        # token type embeddings/ segment embeddings
        token_type_ids = _one_id.new_ones((bs, max_length))
        token_type_ids[grid_pos <= object_end] = 0
        token_type_embeddings = self.token_type_embeddings(token_type_ids)

        mask = text_mask.new_zeros((bs, max_length))
        mask[grid_pos <= text_end] = 1

        embeddings = self.postion_encoding(vl_embeddings + token_type_embeddings)
        embeddings = self.embedding_LayerNorm(embeddings)
        embeddings = self.embedding_dropout(embeddings)

        return embeddings, mask, (grid_pos > object_end) & (grid_pos < text_end), (grid_pos > 0) & (grid_pos < object_end)
