...
class Charades(data.Dataset):
    ...
    def __init__(self, split):
        ...
        for line in anno_file:
            ...
            if s_time < e_time:
                ...
                concept_label = [1. if c in concepts else 0. for c in self.concepts]
                concept_label = np.array(concept_label, dtype=np.float32)

                annotations.append({'video':vid, 'times':[s_time, e_time], 'description': sent, 'duration': self.durations[vid], 'word_vectors': word_vectors, 'concepts':concepts, 'concept_label':concept_label})
    ...
    def __getitem__(self, index):
        ...
        concept_label_list = [self.annotations[index]['concept_label']]
        ...
        while True:
            ...
            if video_id_neg not in select_videos:
                ...
                if sim < 0.9:
                    ...
                    concept_label_list.append(self.annotations[index_neg]['concept_label'])
        ...
        item = {
            ...
            'gt_times': torch.from_numpy(np.array([1., 0., 0., 0., 0.], dtype=np.float32)),
            'concept_label': torch.from_numpy(np.array(concept_label_list, dtype=np.float32))
        }
