...
def collate_fn_train(batch):
    ...
    batch_concept_label = [b['concept_label'].unsqueeze(0) for b in batch]
    ...
    batch_gt_times = torch.cat(batch_gt_times, 0).view(-1)
    batch_concept_label = torch.cat(batch_concept_label, 0)
    batch_concept_label = batch_concept_label.reshape(-1, *batch_concept_label.shape[2:])
    ...
    batch_data = {
        ...
        'batch_gt_times': batch_gt_times,
        'batch_concept_label': batch_concept_label
    }
...
def collate_fn_test(batch):
    ...
    batch_gt_times = torch.cat(batch_gt_times, 0)[:,0]
    batch_concept_label = torch.cat(batch_concept_label, 0)[:,0,:]
